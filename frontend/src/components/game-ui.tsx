...
export function GameButton({ label, onPress, tone = "orange", disabled = false }: { label: string; onPress: () => void; tone?: "orange" | "teal" | "gold"; disabled?: boolean }) {
  ...
  return (
    <Pressable
      ...
      onPress={onPress}
      style={({ pressed }) => [styles.button, { backgroundColor: color }, hovered && styles.buttonHovered, pressed && styles.buttonPressed, focused && styles.focused, disabled && styles.disabled]}
    >
...
export function Hud({ state }: { state: GameState }) {
  return (
    <View style={styles.hud}>
      <View style={styles.hudStat}><Text style={styles.hudLabel}>PLAYER</Text><Text style={styles.hudValue}>{state.playerName}</Text></View>
      <View style={styles.hudStat}><Text style={styles.hudLabel}>ROUND</Text><Text style={styles.hudValue}>{state.round}/8</Text></View>
      <View style={styles.hudStat}><Text style={styles.hudLabel}>CASH</Text><Text style={styles.hudValue}>${state.money}</Text></View>
      <View style={styles.hudStat}><Text style={styles.hudLabel}>REP</Text><Text style={styles.hudValue}>{state.reputation}/10</Text></View>
